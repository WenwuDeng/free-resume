...
这是一个基于 AI 大模型生成的简历生成器。简历生成器用于快速编写、预览并导出 PDF 简历的 Web 应用。你可以在左侧实时编辑简历内容，右侧即时预览排版，并通过浏览器的打印功能保存为 PDF。
...
- 右侧 A4 纸张比例预览，所见即所得
- 一键调用浏览器打印，导出 PDF 简历
- 支持多套主题色切换（蓝 / 绿 / 紫），实时预览
- 支持调整正文字号（小 / 中 / 大），同时作用于屏幕预览和导出 PDF
- 使用 React + TypeScript + Vite + Tailwind CSS 构建

### 效果截图

> 下方截图仅为示意，你可以根据实际简历内容自由调整。

1. 编辑器 + 预览界面

   左侧实时编辑简历内容，右侧按 A4 比例展示最终排版效果，可在顶部切换主题颜色和字号：

   ![编辑 + 预览界面示例](./docs/screenshot-editor-preview.png)

2. 导出 PDF 预览

   点击右上角「保存 PDF」后，在浏览器打印面板中选择「保存为 PDF」，即可导出简历文件：

   ![浏览器打印导出 PDF 示例](./docs/screenshot-print-preview.png)
